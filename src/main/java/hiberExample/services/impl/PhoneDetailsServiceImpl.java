...
public class PhoneDetailsServiceImpl implements PhoneDetailsService {
	...
	@Cacheable(value = "phoneDetails.byProvider", key = "#provider", unless = "#result != null and #result.provider.toUpperCase().startsWith"
            + "('TEST')")
	public PhoneDetails getByProvider(String provider) {
		return phoneDetailsDao.getByProvider(provider);
	}

	@Cacheable(value = "phoneDetails.byTechnology", key = "#technology", unless = "#result != null and #result.provider.toUpperCase()"
            + ".startsWith('TEST')")
